...
1. 管理サーバー（またはソケット）でトークンを発行:
   `/invite-create server:nitac23s name:管理チームA`
...
### Discord スラッシュコマンド
- `/invite-create <srv> <name>`: [管理サーバー限定] 名前付きトークンを発行。
- `/invite-list`: [管理サーバー限定] 有効なトークンを一覧表示。
- `/invite-revoke <token>`: [管理サーバー限定] トークンを失効させる。
